...
/**
 * Bridges Spring's in-process {@link AppointmentCreatedEvent} into
 * a Socket.IO broadcast for any connected admin clients. Mirrors
 * the NestJS gateway's @OnEvent('appointment.created') handler:
 * the broadcast payload is deliberately limited to {@code {id: ...}},
 * never the full appointment.
 *
 * <p>Why ID-only? Two reasons (C1 in the security audit):
 * <ol>
 *   <li>Defence in depth — even if a connection somehow slips past
 *       the cookie auth, no PII is leaked over the wire.</li>
 *   <li>Single source of truth — the admin frontend re-fetches via
 *       the authenticated REST endpoint, which goes through the
 *       same encryption + decrypt path as a normal page load.</li>
 * </ol>
 */
@Component
