...
public class AuditService {
   ...
   * @param appointmentId UUID of the appointment; set to null on delete so the row survives a
   *     CASCADE/SET NULL
   * @param changes key/value map of field changes; encrypted at rest
   * @param context admin id + IP + user agent from the request
   */
  public void log(
      String action, UUID appointmentId, Map<String, Object> changes, AuditContext context) {
    try {
  ...
  /**
   * Clamp the user agent to the column size. User agents in the wild can be arbitrarily long — e.g.
   * browser extensions stacking their tokens on top. Truncating at 512 chars matches the NestJS
   * controller's {@code ua.slice(0, 512)} behaviour.
   */
