...
public class SecurityConfig {
  ...
  /**
   * Content Security Policy directives for an API that only ever serves JSON. Everything is denied
   * by default. Mirrors backend/src/security-config.ts (helmetConfig) in the NestJS backend so the
   * two APIs emit identical CSP headers.
   */
        ...
        .authorizeHttpRequests(
            auth ->
                auth
                    ...
                    .permitAll()
                    // Spring Boot Actuator: /actuator/health and
                    // /actuator/info are public so orchestrators
                    // (Kubernetes, ECS) can probe liveness/readiness
                    // without credentials. All other actuator
                    // endpoints are still gated by .anyRequest()
                    // below and are also NOT exposed via the
                    // management.endpoints.web.exposure.include
                    // allowlist in application.yml.
  ...
  private AuthenticationEntryPoint nestjsStyleEntryPoint() {
    return (HttpServletRequest request,
      ...
      Map<String, Object> body =
          Map.of("statusCode", 401, "message", List.of("Unauthorized"), "error", "Unauthorized");
