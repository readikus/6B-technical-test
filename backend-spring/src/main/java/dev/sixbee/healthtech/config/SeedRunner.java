...
public class SeedRunner implements CommandLineRunner {
    ...
    /**
     * Enforces the same password policy as the NestJS seed:
     * - At least 12 characters
     * - Contains uppercase, lowercase, digit, and symbol
     *
     * Called at seed time so the app refuses to boot with a weak admin
     * password. The login schema is deliberately left permissive so
     * users with weak existing passwords aren't locked out before they
     * can change them — this assertion only applies to freshly seeded
     * passwords. Mirrors backend/seeds/001_admin_user.ts.
     */
