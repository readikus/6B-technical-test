...
public class AppointmentController {
    ...
    public AppointmentResponse create(
        ...
        AuditContext context = AuditContext.anonymous(
    ...
    /**
     * Build an AuditContext from an authenticated request. The
     * admin id is the JWT subject (UUID), the IP comes from the
     * request (honouring X-Forwarded-For for proxied deployments),
     * and the user agent is the User-Agent header if present.
     * Mirrors {@code buildAuditContext} in NestJS
    ...
    /**
     * Trust one proxy hop, same as NestJS {@code app.set('trust
     * proxy', 1)}. X-Forwarded-For takes precedence over
     * request.getRemoteAddr; if it has multiple comma-separated
     * values, the left-most entry is the original client.
     */
