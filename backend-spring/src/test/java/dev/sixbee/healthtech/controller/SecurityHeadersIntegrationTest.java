...
/**
 * Locks in the defence-in-depth HTTP headers the NestJS audit
 * applied via Helmet (M5). Spring Boot must emit the same CSP,
 * Cross-Origin-Resource-Policy, and Referrer-Policy so a browser
 * talking to either backend gets the same security posture.
 *
 * The API only ever serves JSON, so {@code default-src 'none'} is
 * correct — it costs nothing and provides a free second line of
 * defence against reflected XSS.
 */
@SpringBootTest
@ActiveProfiles("test")
