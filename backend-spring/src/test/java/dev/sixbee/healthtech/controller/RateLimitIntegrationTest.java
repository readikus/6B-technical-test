...
/**
 * Exercises the real filter chain with tight rate limits to verify
 * H2: login endpoint rate limiting, global fallback, and that the
 * 429 response shape matches the NestJS throttler output.
 *
 * Overrides the test-default throttle limits via @TestPropertySource
 * so this file can hit the 429 behaviour without affecting the
 * other integration tests, which rely on the generous default in
...
class RateLimitIntegrationTest {
    ...
    void loginThrottlesAfterFiveAttemptsReturns429() throws Exception {
        ...
        byte[] badPayload = objectMapper.writeValueAsBytes(new LoginRequest(ADMIN_EMAIL, "wrong-password"));
        for (int i = 1; i <= 5; i++) {
    ...
    void successfulLoginStillCountsAgainstTheLimit() throws Exception {
        ...
        byte[] goodPayload = objectMapper.writeValueAsBytes(new LoginRequest(ADMIN_EMAIL, ADMIN_PASSWORD));
        for (int i = 0; i < 5; i++) {
            mockMvc.perform(post("/auth/login")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(goodPayload))
